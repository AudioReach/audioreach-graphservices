...
*     modules to interface and use the protocol(ATP protocol) which works
*     on top of diag.
...
/*
      ...
      --------------------
 */
#define LOCAL_DIAG_MAX_RX_PKT_SIZ 15 * 1024
...
*/
    void (*receive_req_buffer_ptr)
        (
         uint8_t *req_buf_ptr, uint32_t req_buf_length,
         uint8_t **resp_buf_ptr, uint32_t *resp_buf_length
        );
